...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        images = lora__impl__.data.load_and_transform_vision_data([img_path], self.device, to_tensor=False)

        if self.transform is not None:
            image = images[0]
            images = self.transform(image)

        texts = lora__impl__.data.load_and_transform_text([class_text], self.device)
